...
let CopyFilePath = [['lib/es/typings', 'lib/cjs/typings'], ['lib/es/global.d.ts', 'lib/cjs/global.d.ts']]

function getFiles (OriginFilePath, CopyFilePath) {
  fs.stat(OriginFilePath, function(err, stat) {
    // 判断是否文件夹
    if (stat.isDirectory()) {
      //读取newFile文件夹下的文件
...
OriginFilePath.forEach((item, index) => {
  CopyFilePath[index].forEach(each => {
    ...
    if (!fs.existsSync(path.resolve(process.cwd(), each)) && !item.isFile) {
      fs.mkdir(path.resolve(process.cwd(), each), err => {
        // console.log(err)
      })
    }
    getFiles(path.resolve(process.cwd(), item.path), path.resolve(process.cwd(), each))
  })
})