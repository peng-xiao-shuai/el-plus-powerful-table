...
const Text = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const develop = ref(Array(props.listLength).fill(false))
    return () => (
      <div style={{display: 'flex', alignItems: 'center', width: '100%', justifyContent: justifyFun(props.aligning)}}>
        <div class={{ content: develop.value[props.index || 0] }}>
          {/* <!-- 主体内容 --> */}
          <div
            style={props.prop.data && props.prop.data.develop ? {
              display: '-webkit-box',
              overflow: 'hidden',
          ...
          >
            {
              props.prop.data && typeof props.prop.data.customFilterFun == "function"
